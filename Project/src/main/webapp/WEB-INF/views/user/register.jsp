	...
	<div class="container col-sm-4 card" align="center">
		...
		<form class="form-group" action="/user/register" method="post"
			...
			<table class="col-sm-10">
				<tr>
					<td>
						<!-- <span class="form-control"></span> --> <b>아이디</b>
						<div class="input-group">
							<input type="text" class="col form-control" id="users_id"
								name="users_id" placeholder="아이디" required="required"
								autofocus="autofocus" maxlength="19"
								onkeypress="if(event.keyCode ==32) event.returnValue=false;" />
							<div class="input-group-append">
								<button class="col btn btn-primary" id="idBtn" type="button">중복
									확인</button>
							</div>
						</div>
						<div class="chk_register" id="chk_id"></div> <input type="hidden"
						class="chkD" id="chk_idD"></input>
					</td>
				</tr>
				<tr>
					<td><b>비밀번호</b> <input type="password" class="form-control"
						id="password" name="password" placeholder="비밀번호"
						required="required" maxlength="19" />
						<div class="chk_register" id="chk_pw1"></div></td>
				</tr>
				<tr>
					<td><b>비밀번호 확인</b> <input type="password" class="form-control"
						id="passwordConfirm" name="passwordConfirm" placeholder="비밀번호 재입력"
						required="required" maxlength="19" />
						<div class="chk_register" id="chk_pw2"></div></td>
				</tr>
				<tr>
					<td>
						<!--기능구현할때 전에 입력한 이름,전화번호 같이 넘어와야함 --> <b>이름</b> <input
						type="text" class="form-control" id="users_name" name="users_name"
				...
				<tr>
					<td><b>닉네임</b>
						<div class="input-group">
							<input type="text" class="form-control" id="users_nickname"
								name="users_nickname" placeholder="닉네임" required="required"
								maxlength="5"
								onkeypress="if(event.keyCode ==32) event.returnValue=false;" />
							<div class="input-group-append">
								<button class="col btn btn-primary" id="nicknameBtn"
									type="button">중복 확인</button>
							</div>
						</div>
						<div class="chk_register" id="chk_nickname"></div> <input
						type="hidden" class="chkD" id="chk_nicknameD"></input></td>
				</tr>
				<tr>
					<td><b>생일</b> <input type="text" class="form-control"
						id="birth_date" name="birth_date" placeholder="yyyy-MM-dd"
						required="required" maxlength="10" />
						<div class="chk_register" id="chk_birth"></div></td>
				</tr>
				<tr>
					<td><b>핸드폰</b>
						<div class="input-group">
							<input type="text" class="form-control" id="users_phone"
								name="users_phone" required="required" maxlength="13"
								onkeypress="if(event.keyCode ==32) event.returnValue=false;" />
							<div class="input-group-append">
								<button class="col btn btn-primary" id="phoneBtn" type="button">중복
									확인</button>
							</div>
						</div>
						<div class="chk_register" id="chk_phone"></div> <input
						type="hidden" class="chkD" id="chk_phoneD"></input></td>
				</tr>
				<tr>
					<td><b>이메일</b> <input type="text" class="form-control"
						id="users_email" name="users_email" value="${users.users_email}"
						readonly="readonly" required="required" />
						<div class="chk_register" id="chk_email"></div></td>
				</tr>
				<tr>
					<td><b>운전 면허증</b>
						<div class="input-group">
							<input type="text" class="form-control" id="license"
								name="license" required="required"
								onkeypress="if(event.keyCode ==32) event.returnValue=false;" />
							<div class="input-group-append">
								<button class="col btn btn-primary" id="licenseBtn"
									type="button">중복 확인</button>
							</div>
						</div>
						<div class="chk_register" id="chk_license"></div> <input
						type="hidden" class="chkD" id="chk_licenseD"></input></td>
				</tr>

			</table>
			<br>
			<div class="container col-sm-10">
				<input type="submit" class="row btn btn-danger" value="가입완료"
					id="registerBtn" name="registerBtn" />
			</div>
...
<script>
	...
	let chk_register = new Array(8).fill(false);
	$('#registerBtn').click(
			function() {
				/* id정규식 */
				if (idV.test($('#users_id').val())
				...
				let validAll = true;
				for (let i = 0; i < chk_register.length; i++) {
