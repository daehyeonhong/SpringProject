...
<!-- <script>
	$(document).ready(function(){
		/* $("#more").click(function(){ */
			/* window.load='href="#collapsebar"'; */
				        $("#more").on("click", function(){ //set이라는 class이름을 가진 태그의 자식 태그를 클릭 할 경우
				            if($(this).hasClass("collapse_show")) { //현재 active라는 class를 가지고 있는가를 확인
				                 $(this).removeClass("collapse_show");  //active가 있으면 제거,
				                 $(this).siblings("collapse_show").slideUp(200); //현재 이벤트에 걸친 애들 중, class이름이 content를 가진 애들을 slideUp한다
				                /*  $(".set > a i").removeClass("fa-minus").addClass("fa-plus"); //fa-minus의 classname을 삭제하고, fa-plus를 추가한다. */
				               }else{ 
